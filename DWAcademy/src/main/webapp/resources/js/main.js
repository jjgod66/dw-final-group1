...
for (var i=0; i<12; i++){
    ...
    b.tl = gsap.timeline({paused:true, repeat:-1})
      .fromTo(b, {y:[-775,1000,1000][column], rotation:-0.05}, {duration:[40,35,26][column], y:[800,-575,-575][column], rotation:0.05, ease:'none'})
      .progress(i%4/4)
}
...
window.onload = function(){
  ...
  var _tl = gsap.timeline({onStart:playBoxes})
      ...
      .set('.mainBoxes',   {left:'45%', xPercent:-50, width:1200, rotationX:14, rotationY:-15, rotationZ:10})
  ...
  $('.photoBox').on('click', function (e){
    if (!isZooming){ //only tween if photoBox isn't currently zooming
      ...
      const isActive = textContainer.classList.contains('active');
	 if (isActive) {
	      // Hide the text container when zooming out
	      gsap.to(textContainer, { duration: 0.2, autoAlpha: 0, onComplete: () => textContainer.classList.remove('active') });
	    } else {
	      // Show the text container and add content when zooming in
	      $('.movieTitle').text('Movie Title'); // Replace 'Movie Title' with the actual movie title
	      $('.movieDescription').text('Movie description goes here.'); // Replace with the actual movie description
	
	      gsap.to(textContainer, { duration: 0.2, autoAlpha: 1, onComplete: () => textContainer.classList.add('active') });
     }
      if (currentImg) { 
        gsap.timeline({defaults:{ease:'expo.inOut'}})
            .to('.mainClose',   {duration:0.1, autoAlpha:0, overwrite:true}, 0)
            .to('.mainBoxes',   {duration:0.5, scale:1, left:'45%', width:1200, rotationX:14, rotationY:-15, rotationZ:10, overwrite:true}, 0)
            .to('.photoBox',    {duration:0.6, opacity:1, ease:'power4.inOut'}, 0)
            .to(currentImg,     {duration:0.6, width:600, height:740, borderRadius:20, x:currentImgProps.x, y:currentImgProps.y, scale:0.5, rotation:0, zIndex:1}, 0)
            // .add(playBoxes, 0.8)
        ...
        gsap.timeline({defaults:{duration:0.6, ease:'expo.inOut'}})
            ...
            .to(currentImg,        {width:'60%', height:'70%', borderRadius:0, x:100, top:0, y:150, scale:1, opacity:1}, 0)
            .to('.mainBoxes',      {duration:0.5, left:'50%', width:'50%', rotationX:0, rotationY:0, rotationZ:0}, 0.15)
